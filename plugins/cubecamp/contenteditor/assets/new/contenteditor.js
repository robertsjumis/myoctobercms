...
/*
var __super = ContentEdit.Image.prototype.html;
ContentEdit.Image.prototype.html = function (indent) {
	this.removeAttr('width');
	this.removeAttr('height');
//	indent = '<p data-ce-tag="img">';
	return __super.call(this, indent);
}
*/
ContentEdit.Image.prototype.html = function(indent) {
	...
	this.removeAttr('width');
	this.removeAttr('height');
	img = "" + indent + '<div class="img-wrapper" data-ce-tag="img">' + "<img" + (this._attributesToString()) + ">" + '</div>';
	if (this.a) {
...
// Patch the behaviour of the Image `html` method to cater for a wrapped image
var _innerFromDOMElement = ContentEdit.Image.fromDOMElement;
ContentEdit.Image.fromDOMElement = function (domElement) {
	var img = domElement.querySelector('img');
	if (img) {
		return _innerFromDOMElement.call(this, img);
	} else {
...
ContentEdit.Video.prototype.html = function (indent) {
	...
	if (this.tagName() == 'video') {
		...
		return indent + '<div class="video-wrapper" data-ce-tag="video">' +
...
var _innerFromDOMElementVideo = ContentEdit.Video.fromDOMElement;
ContentEdit.Video.fromDOMElement = function (domElement) {
	var iframe = domElement.querySelector('iframe');
	if (iframe) {
		return _innerFromDOMElementVideo.call(this, iframe);
	} else {
		return _innerFromDOMElementVideo.call(this, domElement);
	}
}
...
/*
ContentEdit.blockquote.prototype.html = function(indent) {
	...
	return ("" + indent + "<div class='blockquote-wrapper' data-ce-tag='blockquote'><" + this._tagName + attributes + ">" + le) + ("" + indent + ContentEdit.INDENT + this._cached + le) + ("" + indent + "</" + this._tagName + "></div>");
};
*/
...
ContentEdit.blockquote.prototype.html = function(indent) {
	...
	img = "" + indent + '<div class="blockquote-wrapper" data-ce-tag="blockquote">' + "<img" + (this._attributesToString()) + ">" + '</div>';
	if (this.a) {
		le = ContentEdit.LINE_ENDINGS;
		attributes = ContentEdit.attributesToString(this.a);
		attributes = "" + attributes + " data-ce-tag=\"img\"";
...
/*
ContentTools.Tools.impCaption = (function (_super) {
	...
	ContentTools.ToolShelf.stow(tag, 'impCaption');
	tag.label = 'Image Caption';
	tag.icon = 'blockquote';
//	tag.class = 'impCaption';
	tag.tagName = 'caption';

	return tag;

})(ContentTools.Tools.Heading);
 ContentTools.DEFAULT_TOOLS[0].push('impCaption');
 ContentEdit.TagNames.get().register(ContentEdit.Text, 'impCaption')
*/
...
if(typeof ContentToolsSettings !== 'undefined') {
	ContentToolsSettings();
}

$(function () {
	$(".article-content p img").closest("p").addClass("img-wrapper").attr("data-ce-tag", "img");
	$(".article-content div.imgCaption").attr("data-ce-tag", "p");
});
