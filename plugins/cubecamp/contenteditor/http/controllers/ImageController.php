<?php namespace Cubecamp\ContentEditor\Http\Controllers;
...
use Cubecamp\ContentEditor\Http\Middleware\EditorPermissionsMiddleware;
...
/*
			...
			if(strtolower(substr(post('filePath'), '-4')) !== '.svg' && strtolower(substr(post('filePath'), '-4')) !== '.gif') {
				...
				$pathParts = pathinfo(post('filePath'));
				$newFilePath = $pathParts['dirname'] . '/' . $pathParts['filename'] . '-r.' . $pathParts['extension'];
